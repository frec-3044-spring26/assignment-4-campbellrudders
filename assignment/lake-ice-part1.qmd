...
file.exists("C:/Users/campb/OneDrive/Desktop/sophomore year/spring 2026/Environmental Data Science/assignment-4-campbellrudders/data/lake_ice_off_data.xlsx")


ice <- read_excel(file_name, sheet = "data")
```
...
ggplot(sunapee, aes(x = year, y = ice_off_doy)) +
  geom_point() +
  geom_line() +
  labs(title = "Lake Sunapee Ice Off Data", 
       x = "Year", 
       y = "Day of Year") 
```
...
I expect the slope to be negative because in later years, the ice off date is earlier in the year. 
...
The slope is shallow and negative which matches my expectations. 
...
The R2 for this regression is 0.2055, and based on this, `year` and `ice_off_doy` are not strongly related. 
...
The residuals are realtively normally distributed based on the histogram. 

```{r}
ggplot(sunapee) +
  geom_histogram(aes(x = residual), binwidth = 10) +
  theme_bw()+
  labs(title = "Residuals for Sunapee Lake", 
       x = "Residuals", 
       y = "Count")
```
...
Based on the summary above, the p-value for the slope is 0.0000000025. 
...
**Answer 8:**
7 days/ -0.1060227 = about 66 years. The annual change is small and not noticeable year-to-year, the long-term shift is meaningful and consistent with warming trends observed. This data suggests gradual but ecologically important changes in seasonal lake.
...
This model is not very good. With an R-squared of only 20.55% and a slightly left skewed distribution, it is possible that there are better models for this data set. 
...
ggplot(sunapee, aes(x = year, y = ice_off_doy)) +
  geom_point() +
  geom_line() +
  geom_abline(intercept = 318.94856, slope = -0.10602, color = "red") +
  labs(title = "Lake Sunapee Ice Off Data", 
       x = "Year", 
       y = "Day of Year") 
```
...
The predicted ice-off day in 2024 is April 12th (102nd day of the year). The 90% confidence interval is between -19.427 and 14.151. Adding the 5% and 95% residual quantiles gives a 90% confidence interval of, the 83rd day of the year to the 117th day of the year. The confidence interval contains 34 days. 

```{r}
prediction_2040 <- fit$coefficients[1] + 2040 * fit$coefficients[2]

prediction_2040

quantiles <- quantile(fit$residual, probs = c(.05, .95))

quantiles

lower_2040 <- prediction_2040 + quantiles[1]
upper_2040 <- prediction_2040 + quantiles[2]

c(lower_2040, upper_2040)
```
...
The slope for the ice off data before 1970 is represented in the red line. The slope for the ice off data after 1970 is represented in the blue line. Looking at the graph, it is evident that the ice off day is changing more rapidly. The slope went from -0.109668 to -0.3023923, leading to the ice off day after 1970 to be earlier in the year compared to before 1970. 


```{r}
early_sunapee <- sunapee |> 
  filter(year < 1970)

late_sunapee <- sunapee |> 
  filter(year >= 1970)

early_fit = lm(ice_off_doy~year, data = early_sunapee)
late_fit = lm(ice_off_doy~year, data = late_sunapee)

early_sunapee <- early_sunapee |> 
  add_residuals(early_fit) |> 
  add_predictions(early_fit)

late_sunapee <- late_sunapee |> 
  add_residuals(late_fit) |> 
  add_predictions(late_fit)
...
lines_df <- data.frame(
  intercept = c(325.5421, 711.9216),
  slope = c(-0.109668, -0.3023923),
  line_type = c("Before 1970", "After 1970"))
...
ggplot(sunapee, aes(x = year, y = ice_off_doy)) +
  ...
  geom_abline(aes(intercept = intercept, 
              ...
              color = line_type), 
              data = lines_df) +
  scale_color_manual(values = c("Before 1970" = "red", 
                                "After 1970" = "blue")) +
  scale_x_continuous(breaks = seq(1860, 2030, by = 15)) +
  scale_y_continuous(breaks = seq(80, 130, by = 15)) +
  labs(title = "Lake Sunapee Ice Off Data", 
       x = "Year", 
       y = "Day of Year") 
```
